...
def process_2_args(mach_code, args, opcode):
    ...
    if opcode == "mul":
        if len(args) == 3:
            if args[0] in registers and args[2] in registers:
                ...
                mach_code |= registers[args[0]] << 8 #rs
    ...
    if args[1] != ',':
            print('ERROR: Invalid syntax : 7')
            sys.exit(1)

    if len(args) == 3:
        if opcode in ["mov", "mvn", "add", "sub", "rsb", "eor"]:
            # add / sub / rsb / eor 은 rd == rn
            if opcode in ["add", "sub", "rsb", "eor"]:
                mach_code |= registers[args[0]] << 16
                
            if args[2] in registers:
                mach_code |= registers[args[2]]
            elif args[2][0] == '#': #immediate value
                mach_code |= 0b0010 << 24 # I bit -> 1
                args[2] = args[2][1:]
                if int(args[2]) >= 256:
                    #rotate value
            ...
            print("wrong expressions: args = 3")
            sys.exit(1)

    elif len(args) == 5:
        if opcode in ["add", "sub", "rsb", "eor"]:
            if args[3] != ',':
                print("ERROR: Invalid syntax : 8")
                sys.exit(1)
            if args[2] in registers: # rn
                mach_code |= registers[args[2]] << 16
            #rn must be registers
            else: 
                print('ERROR: Invalid operand')
                sys.exit(1)
            ...
            print("wrong expressions: args = 5")
            sys.exit(1)
            ...
            if args[2] in registers:
                ...
                if args[5] in registers:
                    ...
                    if int(args[5]) > 32 or int(args[5]) < 0 :
                        print("ERROR: Invalid syntax : 12")
                        sys.exit(1)
                    
                    mach_code |= int(args[5]) << 7
            else: #rn must be register
                print("ERROR: Invalid syntax : 13")
                sys.exit(1)

        else:
            print("ERROR: Invalid syntax : 14")
            sys.exit(1)
            ...
            # immediate value
                ...
                if int(args[7]) >= 0 and int(args[7]) < 32:
                   mach_code |= int(args[7]) << 7
                    ...
                    print("ERROR: Invalid syntax : 18")
                    sys.exit(1)  
            else:
                print("ERROR: Invalid syntax : 19")
                sys.exit(1)  
                
        else:
            print("ERROR: Invalid syntax : 20")
            sys.exit(1)
...
def rotateIm(im_value):
    ...
    #일반적인 상황
    if length <= 8:
        if length == 8 and (len(bin_list) - k - 1) % 2 == 1: #나머지 길이가 홀수이면 안됨
            print("invalid immediate value: case 1")
            sys.exit(1)
        else:
            if zeros % 2 == 0:
    ...
    #끝과 끝이 연결된 상황
        ...
        if i_len + j_len > 8:
            print("invalid immediate value: case 2")
            sys.exit(1)
        elif (i_len + j_len == 8) and (j_len % 2 == 1):#길이가 8이면서 부분이 홀수
            print("invalid immediate value: case 3")
            sys.exit(1)
...
for line in lines:
    ...
    #Data Section
        ...
        try :
            ...
            print("ERROR: Invalid syntax: 5 ", e)
            sys.exit(1) 
            
#data symbol table making
before = 0
flag = 1
...
for line in two_pass_line:
    ...
    if tokens[0].startswith("b"):
        ...
        result |= registers[tokens[1]] << 12 #rd
        result |= 0b1111 << 16 #rn = pc
